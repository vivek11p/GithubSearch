...
public class ServiceStepDefinitions extends StepDefinitionBase {
  ...
  @And("^the body has response size as (\\d+)$")
  public void theBodyHasResponseSizeAs(int count){
    assertEquals(getScenarioResponseEntity().getBody().toString().split(",\\{").length,count);
  }
